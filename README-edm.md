...
* Python libraries: See [environment.yml](./environment.yml) for exact library dependencies. You can use the following commands with Miniconda3 to create and activate your Python environment:
  ...
  - `conda activate edm`
* VS Code/Cursor users (recommended for local development):
  - Install the Dev Containers extension (`ms-vscode-remote.remote-containers`).
  - Open this repository and use "Reopen in Container". The configuration is in [`.devcontainer/devcontainer.json`](./.devcontainer/devcontainer.json) and reuses the [provided Dockerfile](./Dockerfile).
* Docker users:
...
**VS Code/Cursor dev container (recommended for local development)**:

1. Install the Dev Containers extension (`ms-vscode-remote.remote-containers`) in VS Code or Cursor.
2. Open this repository and choose "Reopen in Container".
3. Run `python example.py` in the container terminal.

The dev container uses the same base image and dependency setup as [`Dockerfile`](./Dockerfile), and still requires the NVIDIA container runtime.

**Docker CLI (fully supported)**: You can run the example script using Docker as follows:
